...
namespace krd {
RenderTriangleMesh::RenderTriangleMesh(FLuxScene *renderScene, Ref<TriangleMesh> const &mesh)
...
std::shared_ptr<RenderTriangleMesh::Resource> RenderTriangleMesh::getResource() {
    ...
    if (vertexBuffer || indexBuffer) {
        // LogTrace("RenderTriangleMesh: resource already created");
    } else {
        ...
        ctx->getDevice()->createBufferResource(
            indexBufferDesc, indices.data(), indexBuffer.writeRef()
        );
    }
