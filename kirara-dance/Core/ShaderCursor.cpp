...
ShaderCursor ShaderCursor::getExplicitCounter() const {
    ...
    if (auto const counterVarLayout = m_typeLayout->getExplicitCounter()) {
...
Result
ShaderCursor::getField(char const *name, char const *nameEnd, ShaderCursor &outCursor) const {
...
static int _peek(char const *slice) {
    char const *b = slice;
    if (!b || !*b)
        return -1;
    return *b;
...
static int _get(char const *&slice) {
    char const *b = slice;
    if (!b || !*b)
        return -1;
    auto result = *b++;
...
Result ShaderCursor::followPath(char const *path, ShaderCursor &ioCursor) {
    ...
    for (;;) {
        ...
        krd::slangCheck(cursor.getField(nameBegin, nameEnd, newCursor));
        cursor = newCursor;
        state = ALLOW_DOT | ALLOW_SUBSCRIPT;
        continue;
    }
