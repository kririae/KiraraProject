...
VKAPI_ATTR VkBool32 VKAPI_CALL debugMessageCallback(
    ...
    size_t /*location*/, int32_t /*msgCode*/, char const *pLayerPrefix, char const *pMsg,
    void * /*pUserData*/
) {
    printf("[%s]: %s\n", pLayerPrefix, pMsg);
    return 1;
...
int initializeVulkanDevice(VulkanAPI &api) {
    // Load vulkan library.
    char const *dynamicLibraryName = "Unknown";
...
int VulkanAPI::findMemoryTypeIndex(uint32_t typeBits, VkMemoryPropertyFlags properties) {
    ...
    int const numMemoryTypes = int(deviceMemoryProperties.memoryTypeCount);
