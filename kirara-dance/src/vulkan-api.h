...
#define VK_API_GLOBAL_PROCS(x)                                                                     \
    x(vkGetInstanceProcAddr) x(vkCreateInstance) x(vkEnumerateInstanceLayerProperties)             \
        x(vkDestroyInstance) /* */

#define VK_API_INSTANCE_PROCS_OPT(x)                                                               \
    x(vkGetPhysicalDeviceFeatures2) x(vkGetPhysicalDeviceProperties2)                              \
        x(vkCreateDebugReportCallbackEXT) x(vkDestroyDebugReportCallbackEXT)                       \
            x(vkDebugReportMessageEXT) /* */

#define VK_API_INSTANCE_PROCS(x)                                                                   \
    x(vkCreateDevice) x(vkDestroyDevice) x(vkEnumeratePhysicalDevices)                             \
        x(vkGetPhysicalDeviceProperties) x(vkGetPhysicalDeviceFeatures)                            \
...
#define VK_API_DEVICE_PROCS(x)                                                                     \
    x(vkCreateDescriptorPool) x(vkDestroyDescriptorPool) x(vkGetDeviceQueue) x(vkQueueSubmit       \
    ) x(vkQueueWaitIdle) x(vkCreateBuffer) x(vkAllocateMemory) x(vkMapMemory) x(vkUnmapMemory)     \
        x(vkCmdCopyBuffer) x(vkDestroyBuffer) x(vkFreeMemory) x(vkCreateDescriptorSetLayout)       \
            x(vkDestroyDescriptorSetLayout) x(vkAllocateDescriptorSets) x(vkUpdateDescriptorSets)  \
                x(vkCreatePipelineLayout) x(vkDestroyPipelineLayout) x(vkCreateComputePipelines)   \
                    x(vkDestroyPipeline) x(vkCreateShaderModule) x(vkDestroyShaderModule)          \
                        x(vkCreateCommandPool) x(vkDestroyCommandPool)                             \
                                                                                                   \
                            x(vkGetBufferMemoryRequirements)                                       \
                                                                                                   \
                                x(vkCmdBindPipeline) x(vkCmdBindDescriptorSets) x(vkCmdDispatch)   \
                                                                                                   \
                                    x(vkFreeCommandBuffers) x(vkAllocateCommandBuffers)            \
                                        x(vkBeginCommandBuffer) x(vkEndCommandBuffer)              \
                                            x(vkBindBufferMemory) /* */

#define VK_API_ALL_GLOBAL_PROCS(x) VK_API_GLOBAL_PROCS(x)

#define VK_API_ALL_INSTANCE_PROCS(x) VK_API_INSTANCE_PROCS(x)
