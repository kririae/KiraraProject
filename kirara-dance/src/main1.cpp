...
public:
    ...
    handleMessage(DebugMessageType type, DebugMessageSource source, char const *message) override {
        char const *typeStr = "";
        switch (type) {
        ...
        char const *sourceStr = "[GraphicsLayer]: ";
        switch (source) {
...
struct HelloWorldExample {
    ...
    size_t const inputElementCount = 16;
    size_t const bufferSize = sizeof(float) * inputElementCount;
...
int HelloWorldExample::createComputePipelineFromShader() {
    ...
    slang::IModule *slangModule = nullptr;
    {
        ...
        slangModule = session->loadModule(
            "/home/krr/Projects/KiraraProject/kirara-dance/src/shaders.slang",
    ...
    shaderCreateInfo.pCode = static_cast<uint32_t const *>(spirvCode->getBufferPointer());
...
int HelloWorldExample::dispatchCompute() {
    ...
    vkAPI.vkCmdBindDescriptorSets(
        commandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, pipelineLayout, 0, 1, &descriptorSet, 0,
        nullptr
    );
