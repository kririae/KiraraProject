...
typedef GLXContext (*glXCreateContextAttribsARBProc)(Display *, GLXFBConfig, GLXContext, int, int const *);
...
public:
    ...
    ~Tutorial00App() { m_pImmediateContext->Flush(); }
    ...
    bool InitVulkan(XCBInfo &xcbInfo) {
        ...
        pFactoryVk->CreateSwapChainVk(
            m_pDevice, m_pImmediateContext, SCDesc, XCBWindow, &m_pSwapChain
        );
    ...
    void Render() {
        ...
        float const ClearColor[] = {0.350f, 0.350f, 0.350f, 1.0f};
        ...
        // Typically we should now call CommitShaderResources(), however shaders in this example
...
XCBInfo InitXCBConnectionAndWindow() {
    ...
    xcb_setup_t const *setup = xcb_get_setup(info.connection);
    ...
    xcb_create_window(
        info.connection, XCB_COPY_FROM_PARENT, info.window, screen->root, 0, 0, info.width,
        info.height, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT, screen->root_visual, value_mask, value_list
    );
    ...
    xcb_change_property(
        info.connection, XCB_PROP_MODE_REPLACE, info.window, (*reply).atom, 4, 32, 1,
        &(*info.atom_wm_delete_window).atom
    );
    free(reply);

    char const *title = "Tutorial00: Hello Linux (Vulkan)";
    xcb_change_property(
        info.connection, XCB_PROP_MODE_REPLACE, info.window, XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8,
        strlen(title), title
    );
    ...
    xcb_change_property(
        info.connection, XCB_PROP_MODE_REPLACE, info.window, XCB_ATOM_WM_NORMAL_HINTS,
        XCB_ATOM_WM_SIZE_HINTS, 32, sizeof(xcb_size_hints_t), &hints
    );
    ...
    // runs
    uint32_t const coords[] = {100, 100};
    xcb_configure_window(
        info.connection, info.window, XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y, coords
    );
    ...
    while (true) {
        ...
        while ((event = xcb_poll_for_event(xcbInfo.connection)) != nullptr) {
            ...
            case XCB_KEY_RELEASE: {
                auto const *keyEvent = reinterpret_cast<xcb_key_release_event_t const *>(event);
...
#define KEY_ESCAPE 0x9
            ...
            case XCB_CONFIGURE_NOTIFY: {
                auto const *cfgEvent =
                    reinterpret_cast<xcb_configure_notify_event_t const *>(event);
...
int x_main() {
    ...
    Window win = XCreateWindow(
        display, RootWindow(display, vi->screen), 0, 0, 1024, 768, 0, vi->depth, InputOutput,
    ...
    glXCreateContextAttribsARBProc glXCreateContextAttribsARB = nullptr;
    {
        ...
        )glXGetProcAddress((GLubyte const *)"glXCreateContextAttribsARB");
    ...
    while (true) {
        ...
        while (XCheckMaskEvent(display, 0xFFFFFFFF, &xev)) {
            ...
            case KeyPress: {
                ...
                char buffer[80];
                int num_char =
                    XLookupString((XKeyEvent *)&xev, buffer, _countof(buffer), &keysym, 0);
                (void)num_char;
...
#error No supported backends
    ...
    if (arg + 1 < argc) {
        ...
        if (strcasecmp(mode, "GL") == 0) {
            ...
            std::cerr
                << mode
                << " is not a valid device type. Only the following types are supported: GL, VK";
            return -1;
        }
    }
