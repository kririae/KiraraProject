...
namespace flux {
namespace detail {
inline constexpr int linearKernelBlockSize = 128;
...
template <LinearIndexFunctor Functor>
__global__ __launch_bounds__(linearKernelBlockSize) void linearKernel(
...
template <detail::LinearIndexFunctor Functor>
void launchLinearKernel(std::size_t elementCount, Functor functor, cudaStream_t stream) {
    if (elementCount == 0)
        return;

    constexpr auto blockSize = static_cast<std::size_t>(detail::linearKernelBlockSize);
    ...
    detail::linearKernel<<<grid, block, 0, stream>>>(elementCount, functor);
