...
protected:
  ...
  bool isReferenceToRange(void const *V, void const *First,
                          void const *Last) const {
  ...
  void assertSafeToReferenceAfterResize([[maybe_unused]] void const *Elt,
...
template <typename T>
SmallVectorImpl<T> &
SmallVectorImpl<T>::operator=(SmallVectorImpl<T> const &RHS) {
...
class KIRA_GSL_OWNER SmallVector : public SmallVectorImpl<T>,
...
public:
  ...
  SmallVector(size_t Size, T const &Value) : SmallVectorImpl<T>(N) {
  ...
  explicit SmallVector(iterator_range<RangeTy> const &R)
      : SmallVectorImpl<T>(N) {
...
inline size_t capacity_in_bytes(SmallVector<T, N> const &X) {
