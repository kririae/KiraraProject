...
// - suppresed warnings by [[maybe_unused]]
// NOLINTBEGIN
...
static_assert(
    sizeof(SmallVector<Struct16B, 0>) >= alignof(Struct16B), "missing padding for 16-byte aligned T"
);
static_assert(
    sizeof(SmallVector<Struct32B, 0>) >= alignof(Struct32B), "missing padding for 32-byte aligned T"
);
static_assert(
...
template <class Size_T>
void *SmallVectorBase<Size_T>::replaceAllocation(
    void *NewElts, size_t TSize, size_t NewCapacity, size_t VSize
) {
...
template class kira::SmallVectorBase<uint32_t>;
...
template class kira::SmallVectorBase<uint64_t>;
